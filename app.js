...
wss.on('connection', (ws) => {
  console.log('On-prem server connected');
  onPremSocket = ws;

  ws.on('message', (message) => {
    console.log('Received from on-prem:', message);
  });

  ws.on('close', () => {
    console.log('On-prem server disconnected');
    onPremSocket = null;
  });
});

app.get('/api/startStream/:AtmID', (req, res) => {
  const { AtmID } = req.params;

  const query = `SELECT AtmID, RouterIp FROM SiteDetail where AtmID = ?;`;
  ...
  connection.query(query, [AtmID], (err, result) => {
    if (err) {
      ...
      if (result.length > 0) {
        if (onPremSocket) {
          // Send message with camera ID to on-prem server
          onPremSocket.send(JSON.stringify( result[0] ));
          res.status(200).send('Message sent to on-prem server');
        } else {
          res.status(500).send('On-prem server not connected');
        }
      }
    }
  });
...
app.get("/checkStatus", verifyToken, (req, res) => {
  ...
  connection.query(
    "SELECT SiteDetail.AtmID, SiteDetail.BranchName, LatestData.ist_evt_dt FROM SiteDetail LEFT JOIN LatestData ON SiteDetail.SiteId = LatestData.SiteId;",
    (err, results) => {
      if (err) {
      ...
      if (results.length > 0) {
        ...
        const atmStatusList = results.map((result) => {
          ...
          } else if (result.ist_evt_dt == null) {
            return {
              AtmID: result.AtmID,
              BranchName: result.BranchName,
              status: "No Data",
      ...
      (err, result) => {
        if (err) {
          ...
          if (result[0]) {
              ...
              (err, result) => {
                if (err) {
                  ...
                  WHERE SiteId = '${SiteId}'`,
                    ...
                    // Updating the IST event time in SiteDetail table
                    connection.query(
                      `UPDATE SiteDetail SET IstTimeStamp = '${moment(
                        istDate
                      ).format(
                        "YYYY-MM-DD HH:mm:ss"
                      )}' WHERE SiteId = '${SiteId}'`,
                    ...
                    connection.query(
                      `INSERT INTO LatestData (macid, SiteId, AtmID, zone1_status, zone2_status, zone3_status, zone4_status, 
                  zone5_status, zone6_status, zone7_status, zone8_status, zone9_status, zone10_status, zone11_status, 
                  zone12_status, zone13_status, zone14_status, zone15_status, zone16_status, zone17_status, zone18_status, 
                  zone19_status, zone20_status, zone21_status, zone22_status, zone23_status, zone24_status, zone25_status, 
                  zone26_status, zone27_status, zone28_status, zone29_status, zone30_status, zone31_status, zone32_status, 
                  panel_evt_dt, ist_evt_dt) 
                  VALUES ('${macid}', '${SiteId}', '${AtmID}', '${z[0]}', '${z[1]
                      }', '${z[2]}', '${z[3]}', '${z[4]}', 
                  '${z[5]}', '${z[6]}', '${z[7]}', '${z[8]}', '${z[9]}', '${z[10]
                      }', '${z[11]}', '${z[12]}', '${z[13]}', 
                  '${z[14]}', '${z[15]}', '${z[16]}', '${z[17]}', '${z[18]
                      }', '${z[19]}', '${z[20]}', '${z[21]}', '${z[22]}', 
            ...
            trans.zone26_Astatus, trans.zone27_Astatus, trans.zone28_Astatus, trans.zone29_Astatus, trans.zone30_Astatus,
            trans.zone31_Astatus, trans.zone32_Astatus FROM SiteDetail AS mst JOIN LatestData AS trans ON mst.SiteId = trans.SiteId WHERE mst.SiteId = '${SiteId}'`,
              (err, zoneresult) => {
                if (err) {
                  console.log(err);
                } else {
                  if (zoneresult[0]) {
                    for (var i = 0; i < 32; i++) {
                      ...
                      var zoneAStatus = zoneAStatusTimeValue
                        ? zoneAStatusTimeValue.split(",")[0]
                        : null;
